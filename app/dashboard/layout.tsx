...
import { getStorageLimit } from '@/lib/plans';
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      const data = await loadProfileAndSubscription(u.id);
      setProfile(data.profile as any);
      setSubscription(data.subscription as any);
    ...
    <div className="min-h-screen bg-[#080B14]">
      ...
      <aside className="hidden lg:fixed lg:flex lg:flex-col lg:w-60 lg:top-16 lg:bottom-0 lg:left-0 bg-white/[0.02] border-r border-white/[0.06] p-4 gap-1">
        ...
        {profile?.is_admin && (
          <Link
            href="/admin"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-amber-400 hover:bg-white/[0.04]"
          >
        ...
        <Link
          href="/"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/[0.04] mt-auto"
        >
      ...
      <button
        ...
        className="lg:hidden fixed top-20 left-4 z-50 p-2 rounded-lg bg-white/[0.06] border border-white/[0.08] text-white/70"
      >
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              animate={{ x: 0 }}
              exit={{ x: -240 }}
              className="fixed top-0 left-0 bottom-0 w-60 bg-[#0F1420] border-r border-white/[0.08] z-50 p-4 pt-6"
            >
              <button
                onClick={() => setMobileOpen(false)}
              ...
              <div className="flex flex-col gap-1 mt-6">
                ...
                {profile?.is_admin && (
                  <Link
                    href="/admin"
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-amber-400"
                  >
                    <Shield className="w-4 h-4" />
                    Admin
                  </Link>
                ...
                <Link
                  href="/"
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/50 hover:text-white mt-auto"
                >
                  <Home className="w-4 h-4" />
                  Home
                </Link>
